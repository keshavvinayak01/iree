...
static LogicalResult setAttentionReductionConfig(
  ...
  // Batched f32 attention with aligned K1 stays on this path: generic
  // Distribute currently miscompiles the small N/K2 tail case. Bail out on very
  // skinny K2 shapes; smaller K2 collapses the reduction below what this path
  // is tuned for.
  if (!keepBatchedF32AttentionOnReductionPath &&
